...
type epicboardRoomActivityType = {
    [key: string]: { // key = sessionEventId
        startTImeStamp: string,
        endTimeStamp: string,
        members: { // Object Array
            [key: string]: { // key = memberID (student/teacher) 
                startTImeStamp: string,
                endTimeStamp: string,
            }[],
        }
...
export const triggerOnCreateEpicboardRoom = functions
    .firestore
    ...
    .onCreate(async (epicboardRoomSnap, context) => {
        ...
        const epicboardRoomData: epicboardRoomObjectType | any = epicboardRoomSnap.data();
