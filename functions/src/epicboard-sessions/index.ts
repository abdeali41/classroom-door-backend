...
import { addModifiedTimeStamp, generateUniqueID, addCreationTimeStamp, pushAsSuccessResponse } from "../libs/generics";
...
export const createEpicboardSession = async (
    ...
    const approvedSessionIdList: string[] = Object.keys(allRequestSlots)
        .filter(slotKey => (!allRequestSlots[slotKey].deleted && !allRequestSlots[slotKey].studentAccepted)) // Filtering Slots which are deleted or not StudentAccepted
        .map(approvedSlotKey => {
            ...
            epicboardSessionBatchWrite.set(epicboardSessionCollection.doc(epicboardSessionId), sessionObj);
    ...
    await createEpicboardRoom(newRoomId, approvedSessionIdList, [studentId], teacherId);

    console.log("Session:: Done Create Room")
    await updateBookingRequestStatus(bookingId, bookingData,
        { epicboardSessionIdList: approvedSessionIdList, roomId: newRoomId });
...
const getAllEpicboardSessionsForUser = async (userId: string) => {
    const userEpicboardSessionsSnapshot = await userEventCollection
        ...
        .get();
    const allBookingData = userEpicboardSessionsSnapshot.docs.map(async epicboardSessionDoc => {
        const epicboardSessionData = epicboardSessionDoc.data();
        console.log("EpicboardSession id::", epicboardSessionData.id, epicboardSessionData)
...
export const handleGetUserEpicboardSession = functions.https.onRequest(async (request: any, response: any) => {
    ...
    getAllEpicboardSessionsForUser(userId)
        .then(data => {
            response.status(200).json(
                pushAsSuccessResponse("Epicboard Sessions Found", data)
            );
        })
        .catch(err => {
            response.status(200).json(
                pushAsSuccessResponse("Epicboard Sessions NOT Found", err)
            );
        })
});
...
export const triggerOnCreateEpicboardSession = functions
    .firestore
    .document(`${firestoreCollectionKeys.EPICBOARD_SESSIONS}/{epicboardSessionId}`)
    .onCreate(async (epicboardSessionSnap, context) => {
        ...
        await updateEpicboardSessionStatus(epicboardSessionId, epicboardSessionData);
...
export const triggerOnUpdateEpicboardSession = functions
    .firestore
    .document(`${firestoreCollectionKeys.EPICBOARD_SESSIONS}/{epicboardSessionId}`)
